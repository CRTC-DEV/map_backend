...
class DeviceTouchScreen extends Model
{
    ...
    public function getDeviceTouchScreenById($id){
        $return = DeviceTouchScreen::where('Id', $id)->first();
        return $return;
    }

    public function getDeviceTouchScreenByDeviceCode($deviceCode) {

        
        $return = DeviceTouchScreen::join('T2Location', 'T2Location.Id', '=', 'DeviceTouchScreen.T2LocationId')
            ->where ('DeviceTouchScreen.Status', '!=', DELETED_FLG)
            ->where('DeviceCode', $deviceCode)->first();
       
        return $return;
    }

    public function getDeviceTouchScreenByDeviceSerial($deviceSerial) {

        
        $return = DeviceTouchScreen::where('DeviceSerial', $deviceSerial)->first();
